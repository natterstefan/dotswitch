...
import { diffEnvMaps, parseEnvContent } from '../lib/parser.js'
  ...
  try {
      ...
      if (options.showValues) {
        output.details = {
          ...
          removed: Object.fromEntries(
            diff.removed.map(k => [k, fromMap.get(k)]),
          ),
          changed: Object.fromEntries(
            diff.changed.map(k => [
              k,
              { from: fromMap.get(k), to: toMap.get(k) },
            ]),
          ),
        }
      }
    ...
    if (diff.changed.length > 0) {
      ...
      for (const key of diff.changed) {
        if (options.showValues) {
          console.log(
            pc.yellow(`  ~ ${key}: ${fromMap.get(key)} → ${toMap.get(key)}`),
          )
        } else {
