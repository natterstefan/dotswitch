...
import { listEnvFiles, switchEnv, getActiveEnv } from "../../src/lib/env.js";
...
const mockedPromptEnvSelection = vi.mocked(promptEnvSelection);

const defaultFiles: EnvFile[] = [
  { name: ".env.staging", env: "staging", path: "/p/.env.staging", active: false },
...
describe("useCommand", () => {
  ...
  it("switches to the specified environment", async () => {
    await useCommand("staging", { force: false, backup: true, dryRun: false, path: "/p" });
  ...
  it("errors when environment not found", async () => {
    ...
    await useCommand("unknown", { force: false, backup: true, dryRun: false, path: "/p" });
  ...
  it("switches even if already active when forced", async () => {
    ...
    await useCommand("staging", { force: true, backup: true, dryRun: false, path: "/p" });

    expect(mockedSwitchEnv).toHaveBeenCalled();
  });

  it("passes backup option through", async () => {
    await useCommand("staging", { force: false, backup: false, dryRun: false, path: "/p" });
  ...
  it("--dry-run shows what would happen without switching", async () => {
    ...
    await useCommand("staging", { force: false, backup: true, dryRun: true, path: "/p" });
  ...
  it("--dry-run without backup skips backup message", async () => {
    ...
    await useCommand("staging", { force: false, backup: false, dryRun: true, path: "/p" });
  ...
  it("errors in non-TTY mode without env argument", async () => {
    ...
    await useCommand(undefined, { force: false, backup: true, dryRun: false, path: "/p" });
